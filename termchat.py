...
def verifier_confiance_tls(host, port, empreinte):
    ...
    Connexions suivantes: on refuse si l'empreinte a change, sauf validation explicite."""
    if not empreinte:
        print(f"{R}⚠️  Aucun certificat recu du serveur — impossible de verifier.{Z}")
        return False
    ...
    if cle not in connus:
        ...
        try:
            with open(KNOWN_HOSTS, "w", encoding="utf-8") as f:
                json.dump(connus := connus, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"{R}⚠️  Impossible d'ecrire {KNOWN_HOSTS}: {e}{Z}")
        return True
...
session = {
    "connecte": False, "nom": None, "numero": None, "pays": None,
    "bio": "", "couleur": "cyan", "statut": "disponible",
    "est_admin": False, "non_lus": 0, "a_pin": False, "pseudo": ""
}
sock_cli = None
en_cours = True
reponses = []
rep_lock = threading.Lock()
phrases_secretes = {}   # numero_contact -> phrase secrete (en memoire seulement)
ma_cle_privee = None    # cle privee X25519 locale
cles_partagees_cache = {}  # numero_contact -> cle Fernet deja derivee via ECDH

def generer_cle(n1, n2, phrase_secrete):
    """Derive une cle Fernet a partir d'une phrase secrete."""
...
def deriver_cle_partagee(ma_cle_privee, cle_publique_pair_b64, n1, n2):
    """Etablit une cle symetrique via ECDH X25519 + HKDF."""
...
def dechiffrer(t64, cle):
    try:
        ...
        return "🔒 [Message illisible - phrase secrete incorrecte]"
...
def afficher_entrant(p):
    ...
    if t == "message":
        ...
        if p.get("chiffre") and session.get("numero"):
            ...
            if cle_auto:
                ...
                try:
                    ...
                    texte = "🔒 [Chiffre - erreur de derivation]"
            else:
        ...
        safe_name = "".join(c for c in (nom_f or "fichier") if c.isalnum() or c in "._-") or "fichier"
        chemin = os.path.join(DOWNLOADS, safe_name); base_, ext = os.path.splitext(safe_name); c = 1
        while os.path.exists(chemin):
            chemin = os.path.join(DOWNLOADS, f"{base_}_{c}{ext}"); c += 1
        contenu_b64 = p.get("contenu", "")
        if not contenu_b64:
            print(f"{R}     ❌ Aucun contenu recu{Z}")
        else:
            try:
        ...
        safe_name = "".join(c for c in (nom_f or "vocal.ogg") if c.isalnum() or c in "._-") or "vocal.ogg"
...
def menu_accueil():
    ...
    print(f"  {C2}2{Z} — 📱  Se connecter (numero + mot de passe)")
...
def inscrire():
    ...
    if rep and rep.get("ok"):
        numero = rep.get("numero"); pays = rep.get("pays",""); pseudo_ok = rep.get("pseudo","")
        print(f"""
 {V}{B}╔══════════════════════════════════════════╗
 ...
 """)
        ...
        erreur(rep.get("msg","Erreur") if rep else "Pas de reponse.")
    entree()
...
def _finaliser_connexion(rep):
    if "nom" not in rep or "numero" not in rep:
        erreur("Reponse serveur incomplete. Reessaie de te connecter.")
        return
    session.update({
    ...
    if session.get("a_pin"):
        ...
        while tentatives < 3:
            ...
            erreur("Trop de tentatives."); session["connecte"] = False; return
    ...
    while en_cours and session.get("connecte"):
        ...
        if convs:
            ...
            for i, c in enumerate(convs, 1):
                badge = f" {R}[{c.get('non_lus',0)}]{Z}" if c.get('non_lus', 0) > 0 else ""
                dm = c.get('dernier_msg', '')
                print(f"  {C2}{i}{Z} — {B}{c.get('nom','?')}{Z}{G} ({c.get('numero','')}){Z}{badge}")
        ...
        if choix == "r": break
            ...
            idx = int(choix) - 1
            if 0 <= idx < len(convs): _ouvrir_chat(convs[idx].get("numero"))
...
def _ouvrir_chat(nd):
    ...
    if cle_pub_pair and ma_cle_privee:
        try:
            cle_chat = deriver_cle_partagee(ma_cle_privee, cle_pub_pair, session.get("numero",""), nd)
    ...
    if not chiffrer_msgs:
        info(f"{u.get('nom','?')} n'a pas encore de cle de chiffrement publiee (ancienne version de TermChat).")
        if input(f"{J}Activer un chiffrement manuel de secours? (o/n): {Z}").strip().lower() == "o":
            phrase = input(f"{J}Phrase secrete partagee avec {u.get('nom','?')} (a se transmettre hors TermChat): {Z}").strip()
            if phrase:
                chiffrer_msgs = True; cle_chat = generer_cle(session.get("numero",""), nd, phrase)
            ...
            for msg in hist:
                ...
                if msg.get("chiffre") and msg.get("type") != "fichier":
                    if cle_chat:
                        ...
                        texte = "🔒 [Chiffre - active le chiffrement pour lire]"
    ...
    while en_cours and session.get("connecte"):
        try: texte = input(f"{B}[→ {u.get('nom','?')}] > {Z}").strip()
        except Exception:
            break
        ...
        if texte.startswith("/auto "):
            try:
                expire_prochain = int(texte.split()[1]); info(f"Prochain message auto-detruit dans {expire_prochain}s.")
            except Exception:
...
def envoyer_fichier_menu():
    ...
    nd = rep.get("user", {}).get("numero")  # on bascule sur le vrai numero pour l'envoi
...
def _envoyer_fichier(nd, chemin):
    ...
    try:
        ...
        erreur(f"Lecture: {e}"); return None
        ...
        if choix == "1":
            ...
            if not groupes: info("Aucun groupe.")
                ...
                for g in groupes:
                    ...
                    print(f"  • {g.get('nom','?')}{adm}  {G}{g.get('membres',0)} membres — ID:{g.get('id','')}{Z}")
            entree()
        elif choix == "2":
            nom = input("Nom du groupe: ").strip()
            if not nom: continue
            envoyer_cli({"action":"creer_groupe","nom":nom}); rep = attendre()
            if rep and rep.get("ok"): succes(f"Groupe '{nom}' cree!"); print(f"   {G}ID: {rep.get('id_groupe')}{Z}")
...
def voir_en_ligne():
    ...
    if not users: info("Personne d'autre en ligne.")
        ...
        for u in users:
            st = STATUTS_ICONS.get(u.get("statut","disponible"), "")
            print(f"  🟢  {u.get('nom','?')}  {G}{u.get('numero','')}{Z}  {st}")
    entree()
...
def mon_profil():
    ...
    pseudo_aff = f"@{session.get('pseudo')}" if session.get("pseudo") else f"{J}(non defini){Z}"
    ...
    if not session.get("pseudo"):
        if input("Definir ton pseudo maintenant? (o/n): ").strip().lower() == "o":
            ...
            if rep and rep.get("ok"): session["pseudo"] = rep.get("pseudo", pseudo); succes(rep.get("msg",""))
            else: erreur(rep.get("msg","?") if rep else "?")
    if input("Modifier ta bio? (o/n): ").strip().lower() == "o":
        ...
        if rep and rep.get("ok"): session["bio"] = bio; succes(rep.get("msg"))
        else: erreur(rep.get("msg","?") if rep else "?")
    entree()
...
def changer_statut():
    ...
    try:
        ...
        if rep and rep.get("ok"): session["statut"] = statut; succes(rep.get("msg"))
        else: erreur(rep.get("msg","?") if rep else "?")
    except Exception:
    ...
    while en_cours and session.get("connecte"):
        ...
        if choix == "1":
            ...
            if c2 == "1":
                ...
                if rep and rep.get("ok"): session["a_pin"] = True; succes(rep.get("msg"))
                else: erreur(rep.get("msg","?") if rep else "?")
            elif c2 == "2":
                envoyer_cli({"action":"supprimer_pin"}); rep = attendre()
                if rep and rep.get("ok"): session["a_pin"] = False; succes(rep.get("msg"))
                else: erreur(rep.get("msg","?") if rep else "?")
            entree()
        elif choix == "5":
            ...
            envoyer_cli({"action":"verifier_mon_abonnement"}); rep = attendre()
        ...
        if choix == "1":
            ...
            if rep and rep.get("ok"):
                ...
                for u in rep.get("users", []):
                    ...
                    print(f"  {st} {u.get('nom','?'):<12} {u.get('numero','')}  {G}{u.get('pays','')}{Z}")
            ...
            numero = input("Numero du compte: ").strip()
            print("  1 — Activer premium mensuel  |  2 — Activer premium annuel  |  3 — Desactiver")
            c2 = input("Choix: ").strip()
            if c2 == "1":
                envoyer_cli({"action":"admin_activer_premium","numero":numero,"type":"mensuel"}); rep = attendre()
                if rep and rep.get("ok"): succes(rep.get("msg",""))
                else: erreur(rep.get("msg","?") if rep else "?")
            elif c2 == "2":
                envoyer_cli({"action":"admin_activer_premium","numero":numero,"type":"annuel"}); rep = attendre()
                if rep and rep.get("ok"): succes(rep.get("msg",""))
                else: erreur(rep.get("msg","?") if rep else "?")
            elif c2 == "3":
                envoyer_cli({"action":"admin_desactiver_premium","numero":numero}); rep = attendre()
                if rep and rep.get("ok"): succes(rep.get("msg",""))
                else: erreur(rep.get("msg","?") if rep else "?")
            entree()
        elif choix == "r": break
    ...
    try:
        while en_cours:
            if not session["connecte"]:
                choix = menu_accueil()
                if choix == "1": inscrire()
                elif choix == "2": connecter_par_numero()
                elif choix == "3": connecter_par_email()
